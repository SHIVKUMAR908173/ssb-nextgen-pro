...
async function run() {
  ...
  // We'll just generate for the first chapter 'nda-calc' as a test run
  const testChapterId = 'nda-calc';
  const chapter = chaptersMap[testChapterId];
  const examId = chapter.study_subjects.exam_id;
  const chapterTopics = topicsData.filter(t => t.chapter_id === testChapterId);
  
  console.log(`Generating real content for ${chapterTopics.length} topics in ${testChapterId}...`);
  ...
  for (const topic of chapterTopics) {
    ...
    try {
      ...
      console.error(`Failed to generate for ${topic.title}: `, e);
    }
  }

  // Rewrite the chapter file
  const examDir = path.join(AUTOGEN_DIR, examId);
  const filePath = path.join(examDir, `${testChapterId}.ts`);
  const varName = `${testChapterId.replace(/-/g, '_').toUpperCase()}_AUTOGEN`;
